    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add_cashier':
                ...
                case 'reset_customer_count':
                    $queues['queue_history'] = []; // Clear the history
                    file_put_contents('data/queues.json', json_encode($queues, JSON_PRETTY_PRINT));
                    $_SESSION['success'] = "Customer count has been reset";
                    header('Location: admin-dashboard.php');
                    exit();
    ...
    <div class="container">
        ...
        <div class="dashboard">
            ...
            <div class="card">
                ...
                <form method="POST" onsubmit="return confirm('Are you sure you want to reset the customer count?');">
                    <input type="hidden" name="action" value="reset_customer_count">
        ...
        <div class="action-buttons">
            ...
            <form method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to reset all queues?');">
...
<div class="window-visibility-section">
    <h2>Select Window/s to Show in Live Queues</h2>
    <form method="POST" action="update-window-visibility.php" onsubmit="return confirm('Update Selected Windows?');">
        <?php 
        ...
        if (file_exists($visibilityFile)) {
            ...
            file_put_contents($visibilityFile, json_encode($defaultVisibility, JSON_PRETTY_PRINT));
        }
        ?>
        ...
        <div class="visibility-options">
            <?php foreach ($allWindows as $window): ?>
                <label class="visibility-option">
                    <input type="checkbox" name="visible_windows[]" value="<?= $window ?>" 
                        <?= $visibility[$window] ? 'checked' : '' ?>>
                    <?= $window ?>
                </label>
            <?php endforeach; ?>
        </div>
        <button type="submit" class="btn">Update Windows</button>
    </form>
</div>

        <h2>Running Text Update</h2>
        <form method="POST" onsubmit="return confirm('Update Running Text?');">
            ...
            <tbody>
                <?php foreach ($cashiers as $cashier): ?>
                    <tr>
                        ...
                        <td>
                            <form method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to remove a cashier?');">
    ...
    <div id="add-cashier-modal" class="modal" style="display: none; position: fixed; z-index: 1; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.4);">
        ...
        function updateDashboardStats() {
            fetch('get-dashboard-stats.php')
                ...
                .then(data => {
                    document.getElementById('total-cashiers').textContent = data.total_cashiers;
                    document.getElementById('total-customers').textContent = data.total_customers;
                    document.getElementById('next-available-window').textContent = data.next_available_window;
                })
